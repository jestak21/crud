...
export class Tab2Page {
  ...
  async getData() {
    try{
      const res = await axios.get('http://localhost/api-crud/api/get_data_mahasiswa.php');
  ...
  async deleteData(id) {
    ...
    fd.append('id', id);
    try{
      const res = await axios.post('http://localhost/api-crud/api/delete_data_mahasiswa.php', fd)
